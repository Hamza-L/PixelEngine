...
PixelImage::PixelImage(const UINT32 width, const UINT32 height, PixelImageFormat format)
    ...
    m_numChannels = format == PixelImageFormat::RGB ? 3 : 4;
...
PixelImage::PixelImage()
    : m_width(1), m_height(1), m_format(PixelImageFormat::UNDEFINED), m_imageName("untitled"), m_fileName("") {
...
void PixelImage::LoadImageFile(const char* filePath) {
    ...
    m_height = (int)height;
    m_numChannels = 4;
    m_format = PixelImageFormat::RGBA; //Most gpu vendors default to RGBA. RGB is not always supported so always default to RGB
...
void PixelImage::LoadEmptyImage(const UINT32 width, const UINT32 height) {
    ...
    m_numChannels = 4;
    m_dataSize = width * height * m_numChannels;
    m_format = PixelImageFormat::RGBA;
}
