...
PixelImage::PixelImage(PixDevice* device, uint32_t width, uint32_t height, bool isSwapChainImage) : m_device(&device->logicalDevice), m_physicalDevice(&device->physicalDevice), m_width(width), m_height(height), m_IsSwapChainImage(isSwapChainImage) {
...
{
    if(m_device == VK_NULL_HANDLE || m_physicalDevice == VK_NULL_HANDLE || m_imageView == VK_NULL_HANDLE)
    {
        return;
    }
    ...
    {
        vkDestroyImage(*m_device, m_image, nullptr); //if swapchain image. it will be destroyed by the swapchain (just like it was created by the swapchain)
...
void PixelImage::createDepthBufferImage()
{
    ...
    m_format = chooseSupportedFormat(*m_physicalDevice,
...
void PixelImage::createImage(VkImageTiling imageTiling, VkImageUsageFlags useFlags, VkMemoryPropertyFlags propFlags) {
    ...
    memoryAllocateInfo.memoryTypeIndex = findMemoryTypeIndex(*m_physicalDevice,
