...
layout (location=2) in float gridSizeFrag;
...
// color of thin lines
vec4 gridColorThin = vec4(0.35, 0.35, 0.35, 1.0);

// color of thick lines (every tenth line)
vec4 gridColorThick = vec4(0.50, 0.50, 0.50, 1.0);
...
vec4 gridColor(vec2 uv, vec2 camPos)
{
    ...
    float opacityFalloff = (1.0 - satf(length(uv) / gridSizeFrag));
